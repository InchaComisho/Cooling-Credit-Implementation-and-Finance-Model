...
EEZ ocean cooling, oxygenation, fishery recovery, and local economic co-benefits.
...
def simulate_scenario(scenario: Scenario) -> pd.DataFrame:
    ...
    water_retention_index = np.clip(100 * (0.20 * urban + 0.45 * humus + 0.25 * forest + 0.35 * water_cycle + 0.08 * ocean_circulation), 0, 100)
    ...
    ocean_surface_cooling_index = np.clip(
        100 * (0.45 * ocean_cooling + 0.25 * ocean_circulation + 0.20 * ocean_oxygenation + 0.10 * water_cycle)
        * scenario.mrv_confidence
        * scenario.ecological_safety,
        0,
        100,
    )
    dissolved_oxygen_recovery_index = np.clip(
        100 * (0.60 * ocean_oxygenation + 0.25 * ocean_circulation + 0.15 * water_cycle)
        * scenario.mrv_confidence
        * scenario.ecological_safety,
        0,
        100,
    )
    marine_food_web_recovery_index = np.clip(
        100 * (0.40 * phytoplankton_fishery + 0.20 * ocean_oxygenation + 0.20 * ocean_circulation + 0.10 * coral_tourism + 0.10 * scenario.ecological_safety)
        * scenario.mrv_confidence,
        0,
        100,
    )
    fishery_tourism_cobenefit_index = np.clip(
        100 * (0.30 * phytoplankton_fishery + 0.20 * coral_tourism + 0.20 * ocean_oxygenation + 0.15 * ocean_cooling + 0.15 * scenario.local_participation)
        * scenario.ecological_safety,
        0,
        100,
    )
    ...
    physical_cooling_index = np.clip(
        100 * (0.22 * urban + 0.15 * humus + 0.15 * forest + 0.23 * water_cycle + 0.25 * (ocean_surface_cooling_index / 100.0))
        * scenario.mrv_confidence,
        0,
        100,
    )
    ...
    disaster_pressure_reduction_index = np.clip(
        100 * (0.18 * urban_heat_island_reduction_index / 100.0 + 0.24 * water_retention_index / 100.0 + 0.20 * evapotranspiration_recovery_index / 100.0 + 0.20 * forest + 0.18 * ocean_surface_cooling_index / 100.0)
        * scenario.mrv_confidence,
        0,
        100,
    )
    ...
    local_economic_cobenefit_index = np.clip(
        100 * (0.16 * waste_to_humus_conversion_rate + 0.16 * humus + 0.16 * forest + 0.12 * urban + 0.18 * fishery_tourism_cobenefit_index / 100.0 + 0.22 * scenario.local_participation),
        0,
        100,
    )

    natural_cooling_function_recovery_factor = np.clip(
        0.30 + 0.20 * humus + 0.20 * forest + 0.25 * water_cycle + 0.20 * ocean_circulation + 0.10 * scenario.local_participation,
        0,
        1.70,
    )
